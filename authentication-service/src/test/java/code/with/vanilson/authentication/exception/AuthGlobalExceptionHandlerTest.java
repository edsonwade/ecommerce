...
/**
 * Unit test for AuthGlobalExceptionHandler — BUG-005 sanitization.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("AuthGlobalExceptionHandler — BUG-005 Error Sanitization")
class AuthGlobalExceptionHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        when(webRequest.getDescription(false)).thenReturn("uri=/api/v1/auth");
    }
    ...
    void handleGeneric_shouldReturnCleanMessage() {
        ...
        when(messageSource.getMessage(eq("auth.error.internal.user"), isNull(), any(Locale.class)))
        ...
        assertThat(body.get("errorCode")).isEqualTo("auth.error.internal");
    ...
    @Test
    @DisplayName("handleGeneric should use 'auth.error.internal.user' key with null args")
    void handleGeneric_shouldUseUserMessageKey() {
        ...
        verify(messageSource).getMessage(eq("auth.error.internal.user"), isNull(), any(Locale.class));
    }
