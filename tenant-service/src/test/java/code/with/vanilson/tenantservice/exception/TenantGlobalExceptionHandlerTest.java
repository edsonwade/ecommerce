...
/**
 * Unit test for TenantGlobalExceptionHandler — BUG-005 sanitization.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("TenantGlobalExceptionHandler — BUG-005 Error Sanitization")
class TenantGlobalExceptionHandlerTest {
    ...
    @InjectMocks
    private TenantGlobalExceptionHandler handler;

    @BeforeEach
    void setUp() {
        when(webRequest.getDescription(false)).thenReturn("uri=/api/v1/tenants");
    }
    ...
    void handleGeneric_shouldReturnCleanMessage() {
        ...
        when(messageSource.getMessage(eq("tenant.error.internal.user"), isNull(), any(Locale.class)))
        ...
        assertThat(body.get("errorCode")).isEqualTo("tenant.error.internal");
    ...
    @Test
    @DisplayName("handleGeneric should use 'tenant.error.internal.user' key with null args")
    void handleGeneric_shouldUseUserMessageKey() {
        ...
        verify(messageSource).getMessage(eq("tenant.error.internal.user"), isNull(), any(Locale.class));
    }
    ...
    void handleNoResourceFound_shouldReturn404() {
        ...
        NoResourceFoundException ex = new NoResourceFoundException(HttpMethod.GET, "/api/v1/tenants/invalid");
